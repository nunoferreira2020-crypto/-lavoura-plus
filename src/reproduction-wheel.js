...
function state(item){
 ...
 if(Number.isFinite(d)&&d>=0&&d<=TERM){
  if(status==='VAZIA'||result.includes('vazia')||result.includes('negativ'))return{key:'red',label:'Vazia',color:'#e31b23',timed:true}
  if(d<=30)return{key:'recent',label:'IA recente',color:'#ffd900',timed:true}
  if(d<=60)return{key:'diag',label:'Diagnóstico',color:'#ff9d00',timed:true}
  if(status==='SECA')return{key:'dry',label:'Seca',color:'#4caf50',timed:true}
  if(Number.isFinite(toCalving)&&toCalving>=0&&toCalving<=21)return{key:'red',label:'Pré-parto',color:'#e31b23',timed:true}
  return{key:'preg',label:'Prenhe',color:'#199ee8',timed:true}
 }
 // Nunca esconder uma vaca. Quando falta uma IA válida, mostrar pelo estado real,
 // mas fora da escala temporal para não inventar uma posição/dia de gestação.
 if(status==='SECA')return{key:'dry',label:'Seca — sem IA atual válida',color:'#4caf50',timed:false}
 if(status==='VAZIA')return{key:'red',label:'Vazia — sem IA atual válida',color:'#e31b23',timed:false}
 if(status==='PRENHE')return{key:'preg',label:'Prenhe — sem IA atual válida',color:'#199ee8',timed:false}
 if(result.includes('vazia')||result.includes('negativ'))return{key:'red',label:'Vazia — sem IA atual válida',color:'#e31b23',timed:false}
 if(result.includes('prenhe')||result.includes('positiv'))return{key:'preg',label:'Prenhe — sem IA atual válida',color:'#199ee8',timed:false}
 return{key:'preg',label:'Sem IA atual válida',color:'#199ee8',timed:false}
}
async function data(){
 ...
 return animals.map(animal=>{const ev=(events||[]).filter(e=>e.animal_id===animal.id);const ia=latest(ev,'IA');const item={animal,ia,day:ia?.event_date?days(ia.event_date,today()):null};item.stage=state(item);return item})
}
function info(host,item){let b=host.querySelector('.rw-info');if(!b){b=document.createElement('div');b.className='rw-info';host.appendChild(b)}const note=item.stage.timed?'':`<div><b>Posição:</b> fora da escala temporal (sem IA atual válida)</div>`;b.innerHTML=`<button class="rw-close" type="button">×</button><strong>🐄 ${item.animal.name||item.animal.number}</strong><div>Brinco: ${item.animal.number}</div><div>Estado: ${item.stage.label}</div><div>Última IA: ${pt(item.ia?.event_date)}</div><div>Secagem prevista: ${pt(item.ia?.expected_dry_off)}</div><div>Parto previsto: ${pt(item.ia?.expected_calving)}</div>${note}`;b.querySelector('.rw-close').onclick=()=>b.remove()}
...
function drawAnimal(svg,host,item,p,deg){const g=el('g',{role:'button',tabindex:'0','aria-label':item.animal.name||item.animal.number});g.style.cursor='pointer';g.appendChild(el('circle',{cx:p.x,cy:p.y,r:'7',fill:item.stage.color,stroke:'#111','stroke-width':'1'}));const left=((deg%360)+360)%360>180;const text=el('text',{x:p.x+(left?-9:9),y:p.y+3,'text-anchor':left?'end':'start','font-size':'8.7','font-weight':'700',fill:'#111'});let label=(item.animal.name||String(item.animal.number)).trim();if(label.length>12)label=label.slice(0,11)+'…';text.textContent=label;g.appendChild(text);g.onclick=()=>info(host,item);g.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();info(host,item)}};svg.appendChild(g)}
function drawWheel(items){
 ...
 const timed=items.filter(x=>x.stage.timed)
 const untimed=items.filter(x=>!x.stage.timed).sort((a,b)=>String(a.animal.name||'').localeCompare(String(b.animal.name||''),'pt'))
 laneItems(timed).forEach(item=>{const deg=START+item.day/TERM*360,r=212-item.lane*24,p=polar(r,deg);drawAnimal(svg,host,item,p,deg)})
 // Animais sem IA válida ficam num anel interior próprio, mantendo a cor do estado.
 // Assim aparecem todos, sem falsificar dias de gestação.
 untimed.forEach((item,i)=>{const deg=300+(i/Math.max(untimed.length,1))*120;const r=105+(i%2)*18;const p=polar(r,deg);drawAnimal(svg,host,item,p,deg)})
 ...
 if(untimed.length){const note=document.createElement('div');note.className='rw-note';note.textContent=`${untimed.length} vaca${untimed.length===1?'':'s'} sem IA atual válida aparece${untimed.length===1?'':'m'} no anel interior, sem inventar uma posição temporal.`;host.appendChild(note)}
 return host
}
