...
Deno.serve(async request => {
  ...
  try {
    ...
    const validValue = (name: string, value: unknown) => {
      ...
      return !['somatic_cells', 'cfu'].includes(name) || Number.isInteger(value)
    }
    const fields = Object.fromEntries(
      Object.entries(extracted).map(([name, field]) => {
        const candidate = field as { value: unknown; confidence: number }
        return [name, candidate.confidence >= threshold && validValue(name, candidate.value)
          ? candidate
          : { value: null, confidence: candidate.confidence }]
      })
    )
